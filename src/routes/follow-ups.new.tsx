...
type FollowUpPatient = {
  ...
  age?: number;
  gender?: string | null;
  contactId?: PatientRecord["contactId"];
  phone?: string;
  whatsappNo?: string;
  email?: string;
};
...
function NewFollowUpPage() {
  ...
  const specialties = Array.from(
    new Set(availableDoctors.map((doctor) => doctor.speciality).filter(Boolean)),
  ).sort();
    ...
    Appointment.getAppointmentById(appointmentId)
      .then((response) => {
        ...
        const appointmentPatient =
          typeof appointment.patientId === "object" ? appointment.patientId : undefined;
        const contact =
          typeof appointment.contactId === "object" ? appointment.contactId : undefined;
        ...
        const appointmentCheckup =
          typeof appointment.checkupId === "object" ? appointment.checkupId : undefined;
  ...
  const handlePatientSearch = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setFound(
        ((response.patients ?? []) as PatientRecord[]).map((item) => ({
          ...item,
  ...
  const handleSubmit = async () => {
    ...
    if (handlerType === "nurse" && !followUpTime)
      return setError("Please select a follow-up time.");
    if (handlerType === "doctor" && (!doctorId || !selectedSlot))
      return setError("Please select a doctor and available slot.");

    setError("");
    setIsSubmitting(true);
    try {
      const parentCheckup =
        typeof parentAppointment?.checkupId === "object" ? parentAppointment.checkupId : undefined;
      const checkupId =
        parentCheckup?._id ||
        (handlerType === "doctor"
          ? (
              await CheckupService.findOrCreate({
      ...
      if (deliveryStatus === "failed" || deliveryStatus === "skipped") {
        setDeliveryWarning(
          response.followUp?.confirmationError || "The WhatsApp confirmation could not be sent.",
        );
      }
      setDone(true);
    } catch (err) {
        ...
        <div className="mx-auto max-w-xl">
          <Card className="p-8 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              The follow-up has been saved successfully.
            </p>
            {deliveryWarning && (
    ...
    <AppShell breadcrumbs={[{ label: "Dashboard", to: "/dashboard" }, { label: "New Follow-up" }]}>
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">
          Schedule a doctor follow-up or create a nurse follow-up visit.
        </p>
      </div>
                ...
                <div className="mt-2 grid gap-2 sm:grid-cols-2">
                  {(["doctor", "nurse"] as HandlerType[]).map((value) => (
                    <button
                      ...
                      className={cn(
                        "rounded-lg border px-3 py-3 text-left text-sm font-medium capitalize transition-all",
                        handlerType === value
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <select
                    ...
                    value={reason}
                    onChange={(event) => setReason(event.target.value as FollowUpReason)}
                  >
                    {REASONS.map((item) => (
            ...
            {patient ? (
              <div className="flex flex-col items-center text-center">
                ...
                <div className="mt-3 text-xs text-muted-foreground">
                  {patient.phone || patient.whatsappNo || patient.email || "No contact saved"}
                </div>
              </div>
            ) : (
...
function PatientSearch(props: {
  ...
  found: FollowUpPatient[];
  hasSearched: boolean;
  isSearching: boolean;
  onSearch: (event: FormEvent<HTMLFormElement>) => void;
  onSelect: (patient: FollowUpPatient) => void;
}) {
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Search by phone and select a patient for this follow-up.
        </p>
      </div>
        ...
        <>
            ...
            {weekDates.map((date) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "rounded-lg border p-3 text-left transition-all disabled:cursor-not-allowed disabled:opacity-50",
          ...
          {isLoadingSlots ? (
            ...
            <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
              No free slots are available for this day.
            </div>
          ) : null}
          <div>
            ...
            <Input
              ...
              onChange={(event) =>
                setPatientReminderMinutes(Math.max(0, Number(event.target.value) || 0))
              }
            />
          </div>
        </>
